package com.jrzx.platform.cxc.common.web.listener;

import com.google.common.collect.Maps;
import com.jr.platform.core.constant.CommonConstants;
...
public class RequestMappingScanListener implements ApplicationListener<ApplicationReadyEvent> {
    ...
    public void onApplicationEvent(ApplicationReadyEvent event) {
        try{
            ...
            res.put("list",list);
            redisService.hmSet(CommonConstants.JR_API_RESOURCE, microService, res,18000L);
            redisService.add(CommonConstants.JR_SERVICE_RESOURCE, microService,18000L);
            log.info("资源扫描结果:serviceId=[{}] size=[{}] redis缓存key=[{}]", microService,  list.size(), CommonConstants.JR_API_RESOURCE);
        }catch (Exception e){
            log.error("扫描资源服务报错：{}",e);
        }
    }
